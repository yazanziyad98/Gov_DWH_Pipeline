...
1. **One speed, one machine.** SSIS executed packages at a fixed throughput ceiling with no horizontal scaling path — you could tune buffer sizes and upgrade the host, but you could not add nodes. NiFi scales by adding processors and clustering; Spark scales by adding executors across NodeManagers. SSIS scales by buying a bigger box. There was also no native path to *streaming* — every change to source data had to wait for the next package run.
