...
The legacy stack was **SQL Server + SSIS**. It worked for years, but as the customer's needs evolved past nightly batch reporting, four specific limitations dominated:

1. **One speed.** SSIS executed packages at a fixed throughput ceiling. There was no native path to *streaming* — every change to source data had to wait for the next package run.
...
flowchart LR
    subgraph EDGE["🛰️ Edge server"]
        direction TB
        SRC[("Source MySQL<br/>system of record")]
        MINIFI["MiNiFi Java<br/>managed by CEFM"]
        CSV[("CSV backup snapshots<br/>customer cron-maintained")]
        SRC -.->|JDBC| MINIFI
        CSV -.->|FetchFile<br/>on failure| MINIFI
    end

    subgraph CLUSTER["🏢 Cluster network"]
        direction TB
        NIFI["NiFi Cluster · 2 nodes<br/>Site-to-Site Input Port"]
        STAGING[("Staging MySQL<br/>database: gov")]
        AIRFLOW["Airflow 3.0.6<br/>@daily DAG"]
        YARN["Cloudera CDP YARN<br/>3 NodeManagers"]
        SPARK["PySpark job<br/>--master yarn"]
        MINIO["MinIO Cluster<br/>3 nodes · erasure-coded"]
        BRONZE[("🥉 gov.data<br/>Bronze · Parquet")]
        GOLD[("🥇 gov.data.gold<br/>Gold · Iceberg")]
        NIFI -->|PutDatabaseRecord| STAGING
        AIRFLOW -->|BashOperator<br/>spark-submit| YARN
        YARN --> SPARK
        SPARK -->|15-way partitioned<br/>JDBC read| STAGING
        SPARK -->|s3a:// writes| MINIO
        MINIO --- BRONZE
        MINIO --- GOLD
    end

    MINIFI ==>|Site-to-Site<br/>over HTTP| NIFI

    classDef edge fill:#fce4ec,stroke:#c2185b,color:#880e4f
    classDef cluster fill:#fff3e0,stroke:#e65100,color:#bf360c
    class EDGE edge
